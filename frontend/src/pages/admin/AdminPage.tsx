...
const AdminPage = () => {
	...
	const handleSeedDatabase = async () => {
		try {
			...
			setSeedMessage(`✅ Database seeded! ${response.data.stats.songsCount} songs and ${response.data.stats.albumsCount} albums created.`);
			...
			const errorMsg = error.response?.data?.message || "Failed to seed database";
				...
				<div className='flex items-center justify-between'>
					<div className='flex items-center gap-3'>
						...
						<div>
							...
							<p className='text-sm text-zinc-400'>Load default songs and albums</p>
						</div>
					</div>
					<Button
